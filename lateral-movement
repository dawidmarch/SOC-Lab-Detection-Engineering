...
## 2. Metodologia ataku
Wykorzystano narzędzie `impacket-wmiexec` do zdalnego wykonania kodu na hoście docelowym `Windows10-Target`.
...
* **Dlaczego nie było logu "Process Create" dla wmiprvse.exe?** 
  Usługa `wmiprvse.exe` (WMI Provider Host) jest usługą systemową działającą w tle. W momencie ataku nie powstaje nowy proces `wmiprvse.exe`, lecz uruchamia on proces potomny (`cmd.exe`). Sysmon poprawnie zalogował proces potomny, co pozwoliło na identyfikację podejrzanej ścieżki w `parentCommandLine`.
